...
from agent_interrogator.config import OutputMode, InterrogationConfig, ModelProvider, LLMConfig
        ...
        def print(self, *args, **kwargs):
            ...
            if args:
                ...
                if hasattr(obj, '__rich_console__'):
                    ...
                    segments = list(obj.__rich_console__(real_console, real_console.options))
...
async def test_interrogator_output_modes(mock_console):
    ...
    for mode in OutputMode:
        ...
        print(f"\nMode: {mode}")
        print(f"Captured output: {mock_console.output}")
        
        if mode == OutputMode.QUIET:
            assert len(mock_console.output) == 0, f"Expected no output in quiet mode, got: {mock_console.output}"
        else:
            # Should have startup info (logo, config table, ready message)
            assert len(mock_console.output) >= 3, f"Expected at least 3 outputs, got {len(mock_console.output)}: {mock_console.output}"
            assert any("Agent Interrogator Configuration" in out for out in mock_console.output), "Configuration table not found in output"
            assert any("Ready to begin interrogation" in out for out in mock_console.output), "Ready message not found in output"
